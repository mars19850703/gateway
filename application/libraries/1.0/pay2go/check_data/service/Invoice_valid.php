...
class Invoice_valid extends BaseCheck
{
    ...
    public function invalid(array $data)
    {
        ...
        if (isset($data['InvoiceNumber']) && !empty($data['InvoiceNumber'])) {
            if (strlen($data['InvoiceNumber']) !== 10) {
                return 'INVOICE_61000';
            }
        } else {
            return 'INVOICE_61001';
        }

        // 發票作廢理由
        if (!isset($data['InvalidReason']) || empty($data['InvalidReason'])) {
            $data['InvalidReason'] = '測試測試';
        } else {
            if (mb_strlen($data['InvalidReason']) > 20) {
                return 'INVOICE_61003';
            }
        }
    ...
    public function allowance(array $data)
    {
        // 發票號碼
        if (isset($data['InvoiceNumber']) && !empty($data['InvoiceNumber'])) {
            if (strlen($data['InvoiceNumber']) !== 10) {
        ...
        if (isset($data['TotalAmount'])) {
            if (!$this->validAmount($data['TotalAmount'])) {
                return 'INVOICE_62001';
            } else {
                $totalAmount = 0;
                foreach ($data['ItemTaxAmount'] as $taxAmount) {
                    $totalAmount += intval($taxAmount);
                }
                if (intval($data['TotalAmount']) !== $totalAmount) {
                    return 'INVOICE_62001';
                }
            }
        } else {
        ...
        // 商品稅額
        if (!isset($data['ItemTaxAmount']) || !is_array($data['ItemTaxAmount']) || count($data['ItemTaxAmount']) === 0) {
            return 'INVOICE_62000';
        } else {
            if (count($data['ItemTaxAmount']) === $itemCount) {
                foreach ($data['ItemTaxAmount'] as $key => $amount) {
                    if (!is_numeric($amount)) {
                        return 'INVOICE_62000';
                    }
                }
